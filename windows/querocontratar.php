<div class="container-fluid">
    <div class="row align-items-center">

        <div class="lh-3 texto mx-auto text-center text-md-start col-9 col-sm-6 col-md-6 pt-5">

            <h1 class="fontsize-6 fw-normal">
                <strong class="textoGradiente fw-semibold fontsize-6">Profissionais independentes</strong> altamente qualificados disponíveis para iniciar seu projeto <strong class="textoGradiente fw-semibold fontsize-6">ainda hoje.</strong>
            </h1>
            <h2 class="mt-3 lh-2 fw-normal pb-3 pb-md-0 mb-md-3 display-6 fontsize-1">Conecte-se com uma extensa rede de profissionais autônomos que estão no mesmo fuso horário e próximos a você.</h2>
            <a href="#" class="rounded-5 btn fundoGradiente fontsize-1 sombra btn-servicos">Eu quero me conectar</a>
        </div>

        <div class="fundo col-8 col-sm-4 col-xl-3 m-auto pt-5 maxwidth1">
            <img src="img/profissional.png" alt="Imagem Profissional" class="img-fluid imagem">
        </div>

    </div>
    <div class="row">
        <div class="col-12 espaco">
            <h1 class="textosombra text-center fw-bold fontsize-6 textoGradiente col-12 mx-auto">Serviços Solicitados</h1>
        </div>
    </div>

    <div class="row pt-5 pb-5 text-center">

        <div class="col-12 mt-4">
            <div class="swiper sliderProfissionais col-10 rounded">
                <div class="swiper-wrapper text-center col-10">
                    <div class="swiper-slide pe-1">
                        <div class="card mx-auto sombra maxwidth3">
                            <img src="https://img.freepik.com/free-photo/smiling-carpenter-measuring-wooden-planks_329181-3745.jpg?w=740&t=st=1706037365~exp=1706037965~hmac=f639710bd89286286d95fa41b2456eddb18ead63c0ca0757e473254207329a26" class="card-img-top img-fluid " alt="Imagem">
                            <div class="card-body">
                                <h4 class="card-title textoGradiente fontsize-1">Montagem de móveis</h4>
                            </div>
                        </div>
                    </div>
                    <div class="swiper-slide pe-1">
                        <div class="card mx-auto sombra maxwidth3">
                            <img src="https://img.freepik.com/free-photo/smiling-carpenter-measuring-wooden-planks_329181-3745.jpg?w=740&t=st=1706037365~exp=1706037965~hmac=f639710bd89286286d95fa41b2456eddb18ead63c0ca0757e473254207329a26" class="card-img-top img-fluid " alt="Imagem">
                            <div class="card-body">
                                <h4 class="card-title textoGradiente fontsize-1">Montagem de móveis</h4>
                            </div>
                        </div>
                    </div>
                    <div class="swiper-slide pe-1">
                        <div class="card mx-auto sombra maxwidth3">
                            <img src="https://img.freepik.com/free-photo/young-craftsman-building-house_1303-27969.jpg?w=740&t=st=1706037708~exp=1706038308~hmac=34c2e1e1b095be4944e5fec464630d304392520d91c21cc5272123bebfe18530" class="card-img-top img-fluid " alt="Imagem" width="300px" height="300px">
                            <div class="card-body">
                                <h4 class="card-title textoGradiente fontsize-1">Serviço de Pedreiro</h4>
                            </div>
                        </div>
                    </div>
                    <div class="swiper-slide pe-1">
                        <div class="card mx-auto sombra maxwidth3">
                            <img src="https://img.freepik.com/free-photo/man-electrical-technician-working-switchboard-with-fuses_169016-24062.jpg?w=740&t=st=1706037587~exp=1706038187~hmac=4f1345f90702be558cea88657e46f551860314677f56a3194c932ffcfed6d9a0" class="card-img-top img-fluid " alt="Imagem" width="300px" height="300px">
                            <div class="card-body">
                                <h4 class="card-title textoGradiente fontsize-1">Serviço de Eletricista</h4>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="swiper-pagination"></div>

                <div class="swiper-button-prev textoGradiente fontsize-5"></div>
                <div class="swiper-button-next textoGradiente fontsize-5"></div>
            </div>

        </div>
        <script type="module">
            import Swiper from 'https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.mjs'

            const sliderProfissionais = new Swiper('.sliderProfissionais', {
                effect: 'coverflow',
                coverflowEffect: {
                    rotate: 5,
                    slideShadows: false,
                },
                grabCursor: true,
                keyboard: {
                    enabled: true,
                    onlyInViewport: false,
                },
                autoplay: {
                    delay: 4000,
                },
                slidesPerView: 1,
                spaceBetween: 30,
                pagination: {
                    el: ".swiper-paginatio",
                    clickable: true,
                },
                breakpoints: {
                    500: {
                        slidesPerView: 2,
                        spaceBetween: 20,
                    },
                    768: {
                        slidesPerView: 2,
                        spaceBetween: 40,
                    },
                    1024: {
                        slidesPerView: 3,
                        spaceBetween: 50,
                    },
                    1800: {
                        slidesPerView: 4,
                        spaceBetween: 60,
                    },
                },
                loop: true,
                navigation: {
                    nextEl: ".swiper-button-next",
                    prevEl: ".swiper-button-prev",
                },

            });
        </script>

    </div>

    <div class="row mt-4 mb-5">
        <div class="col-8 mx-auto text-center">
            <h3 class="fw-normal fontsize-2 lh-5">A <strong class="textoGradiente fw-semibold">liderança</strong> da nossa comunidade de talentos na região é incontestável. Descubra profissionais de <strong class="textoGradiente fw-semibold">alta qualidade</strong>, com habilidades comprovadas e prontos para iniciar o trabalho <strong class="textoGradiente fw-semibold">ainda hoje</strong>.</h3>
        </div>
    </div>

    <div class="row pt-5 pb-5 text-light fundoGradiente">

        <div class="col-12">
            <div class="col-12 text-center mx-auto py-3">
                <img src="img/facilitaaisite.png" alt="Logo" class="col-8 col-sm-4 d-inline text-center maxwidth2">
            </div>
            <div class="col-12 text-center mt-3">
                <h3 class="fontsize-3 col-9 m-auto">É a principal plataforma brasileira para encontrar serviços especializados. Conectamos profissionais de diversas regiões do país a pessoas que necessitam de serviços, assegurando atendimento de qualidade, praticidade e rapidez para atender a uma ampla gama de necessidades.</h3>
            </div>
            <div class="col-12 text-center mt-5">
                <button class="btn btn-light fontsize-3 fw-semibold shadow-lg textoRoxo">Mais informações</button>
            </div>
        </div>

    </div>


</div>